...
class BehaviourDetector:
        ...
        "init": [
            {
                ...
                "message": f"cat=clothing, is M or F in age range"
            }
        ],
        "clothes_visited": [
            {
                ...
                "message": "current-state=clothing, new-cat=shoes"
            },
            # {
            #     "condition": lambda row, current_state: (row["category"] == "clothing"),
            #     "next_state": "clothes_visited",
            #     "message": "current-state=clothing, new-cat=clothing"
            # }
        ],
        "shoes_visited": [
            {
                "condition": lambda row, current_state: (row["category"] == "clothes"
                                                        # and row["productId"] != current_state["rows"][0]["productId"]
                                                        ),
                "next_state": "offer",
                "message": "current-state=shoes, new-cat=offer"
            }
            # ,
            # {
            #     "condition": lambda row, current_state: (row["category"] == "clothing"
            ...
            #                                             ,
            #     "next_state": "clothes_visited",
            #     "message": "current-state=shoes, new-cat=clothing"
            # }
        ]
    }
    ...
    def process_dataframe(self, row: dict):
        user_id = row["userId"]
